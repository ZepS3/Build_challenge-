...
def main():
    ...
    try:
        ...
        print("--- Sales Statistics by Region (summarizingDouble) ---")
        for region, stats in analyzer.get_sales_stats_by_region().items():
            print(f"{region}: count={stats['count']}, sum=${stats['sum']:,.2f}, "
                  f"avg=${stats['mean']:,.2f}, min=${stats['min']:,.2f}, max=${stats['max']:,.2f}")
        print()
